...
@section('content')
@php
    $analyticsIndexRoute = auth()->user()->isSystemAdministrator() ? 'superadmin.analytics' : 'admin.analytics';
    $analyticsReportsRoute = auth()->user()->isSystemAdministrator() ? 'superadmin.reports' : 'admin.reports';
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (printDssButton) {
        printDssButton.addEventListener('click', function () {
            ...
            const stiSeal = @json(asset('Campfix/Images/STI-Academic-Seal-One-Color_400.png'));
            const campfixLogo = @json(asset('Campfix/Images/logo.png'));
            const printableReport = report.cloneNode(true);
            const reportCover = printableReport.querySelector('.dss-report-cover');
            if (reportCover) reportCover.remove();
