...
<p>Reports in this folder can be restored or permanently deleted.</p>
@endsection
...
<div class="container-fluid px-3">
        ...
        <div class="card-body bg-warning bg-opacity-10">
            <div class="row align-items-center">
                ...
                <div class="col-md-4 text-end">
                    <span class="badge bg-warning fs-5">{{ $reports->count() }} reports</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-3">
        <div class="card-body">
            <div class="row align-items-center">
                <div class="col-md-4">
                    <form id="bulkRestoreForm" method="POST" action="{{ route('admin.deletedReports.restoreSelected') }}">
                        @csrf
                        <input type="hidden" name="report_ids" id="selectedReportIds">
                ...
                <div class="col-md-4 text-center">
                    <div class="d-flex align-items-center justify-content-center">
                        <label for="retentionDays" class="me-2 mb-0">Auto-delete after:</label>
                ...
                <div class="col-md-4 text-end">
                    ...
                    <span id="selectedCount" class="text-muted ms-3">0 reports selected</span>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <tbody>
                        @forelse($reports as $report)
                            <tr data-id="{{ $report->id }}">
                                ...
                                <td>{{ $report->title }}</td>
                                <td>{{ $report->category ? $report->category->name : 'N/A' }}</td>
                            ...
                            <div class="modal fade" id="permanentDeleteModal{{ $report->id }}" tabindex="-1">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header bg-danger text-white">
                                            <h5 class="modal-title">Permanently Delete Report</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                        </div>
                                        <div class="modal-body">
                                            <p>Are you sure you want to permanently delete <strong>{{ $report->title }}</strong>?</p>
                                            <p class="text-danger"><i class="fas fa-exclamation-triangle"></i> This action cannot be undone. The report will be permanently removed from the system.</p>
                                        </div>
                            ...
                            <tr>
                                <td colspan="11" class="text-center p-4">
                                    <div class="alert alert-info mb-0">
                                        ...
                                        <h5>No Deleted Reports</h5>
                                        <p class="mb-0">Deleted reports will appear here. You can delete reports from the Reports page.</p>
...
<div class="modal fade" id="permanentDeleteAllModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-danger text-white">
                <h5 class="modal-title">Permanently Delete All Reports</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <p>Are you sure you want to permanently delete all <strong>{{ $reports->count() }}</strong> reports?</p>
                <p class="text-danger"><i class="fas fa-exclamation-triangle"></i> This action cannot be undone. All reports will be permanently removed from the system.</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
...
<script>
    ...
    function updateSelectedCount() {
        ...
        const reportIdsEl = document.getElementById('selectedReportIds');
        
        // Return early if elements don't exist (no reports)
        ...
        reportIdsEl.value = JSON.stringify(selectedIds);
    }
    ...
    function bulkRestore() {
        ...
        if (confirm('Are you sure you want to restore ' + checkboxes.length + ' report(s)?')) {
            document.getElementById('bulkRestoreForm').submit();
        }
    }

    // Context menu functions
    function showContextMenu(e, reportId) {
        e.preventDefault();
    ...
    function contextView() {
        ...
        alert('View functionality for report ' + currentReportId);
    }
    ...
    function contextRestore() {
        ...
        if (confirm('Are you sure you want to restore this report?')) {
            ...
            form.method = 'POST';
            form.action = '/admin/deleted-reports/' + currentReportId + '/restore';
    ...
    function contextPermanentDelete() {
        hideContextMenu();
        const modalId = 'permanentDeleteModal' + currentReportId;
                ...
                if (confirm(`Set auto-filter to show reports deleted more than ${days} days ago?`)) {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            window.location.href = '{{ route("admin.deletedReports") }}?days=' + days;
                        } else {
