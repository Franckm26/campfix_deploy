...
<article class="dss-report" id="dssExecutiveReportContent">
    ...
    <section class="dss-report-section">
        ...
        @if($totalReports > 0)
            ...
            @if($executiveSummary['top_recurring_issue'])<p>The highest-risk recurring problem type is <strong>{{ $executiveSummary['top_recurring_issue']['problem_type'] }}</strong>, recorded {{ number_format($executiveSummary['top_recurring_issue']['count']) }} times with {{ number_format($executiveSummary['top_recurring_issue']['open']) }} still open. This should be addressed through root-cause action rather than treated only as separate incidents.</p>@endif
        @else
