...
<div class="sidebar">
    <div class="sidebar-header">
        <img src="{{ asset('Campfix/Images/logo.png') }}" alt="CampFix Logo" width="160" height="160" fetchpriority="high">
    </div>
    ...
    <div class="sidebar-content">
        ...
        @if(auth()->user()->isSystemAdministrator())
            ...
            <a href="{{ route('admin.logs') }}" class="{{ Request::is('admin/logs*') || Request::is('system-admin/activity-logs*') ? 'active' : '' }}"><i class="fas fa-shield-alt"></i> Forensic Audit Logs</a>
            ...
            @endif
                <div class="notification-dropdown" id="notificationDropdown">
                    <div class="notification-header">
                        ...
                        <span class="notification-count">{{ $unread_count ?? 0 }}</span>
                    </div>
                    <div class="notification-list">
                        @forelse($notifications ?? [] as $notification)
                            <div class="notification-item {{ !$notification->read_at ? 'unread' : '' }}" onclick="openNotificationModal('{{ $notification->id }}', event)">
                                <div class="notification-avatar">
                                    ...
                                    @else
                                        <div class="notification-avatar-initial"><i class="fas fa-bell"></i></div>
                                    @endif
                                </div>
                                ...
                                @endif
                                <button onclick="deleteNotification('{{ $notification->id }}', event)" class="notification-delete-btn" title="Delete notification">
                    ...
                    @if(count($notifications ?? []) > 0)
                        <div class="notification-footer">
                            <a href="#" onclick="openAllNotifications(event)">View all notifications</a>
                        </div>
                    @endif
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function syncDepartmentRequirement() {
        ...
        if (departmentContainer) {
            departmentContainer.style.display = isRequired ? 'block' : 'none';
        }
    ...
    if (previewModal) {
        previewModal.addEventListener('show.bs.modal', function() {
            ...
            // Show the department whenever the configured route requires it.
    ...
    function validateEventRequestForm() {
        ...
        if (!intendedUser || !intendedUser.value) {
            showFieldError(intendedUser, 'Please select an intended user');
            errors.push('Intended user is required');
            isValid = false;
        }
...
document.addEventListener('click', function(event) {
    ...
    if (userDropdown && !event.target.closest('.user-dropdown-top')) {
        userDropdown.classList.remove('show');
    }
    
    if (notificationDropdown && !event.target.closest('.notification-bell')) {
        notificationDropdown.classList.remove('show');
    }
});
...
<div class="modal fade" id="notificationDetailModal" tabindex="-1" aria-labelledby="notificationDetailModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="notificationDetailModalLabel">Notification from <span id="modalSenderName"></span></h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="notification-modal-sender">
                    <img id="modalSenderAvatar" src="" alt="" class="rounded-circle" width="50" height="50" style="display:none;">
                    <div>
                        <div class="fw-bold" id="modalSenderNameFull"></div>
                        <small class="text-muted">@ <span id="modalNotificationTime"></span></small>
                    </div>
                </div>
                <hr>
...
async function openNotificationModal(notificationId, event) {
    ...
    try {
        ...
        const notifItem = document.querySelector(`.notification-item[onclick*="${notificationId}"]`);
        if (notifItem) {
...
async function deleteCurrentNotification() {
    ...
    try {
        ...
        // Remove from list
        const notifItem = document.querySelector(`.notification-item[onclick*="${currentNotificationId}"]`);
...
function openAllNotifications(event) {
    ...
    event.stopPropagation();
    // For now, just show an alert. You can create a dedicated page later
    showNotificationToast('All notifications page - coming soon!', 'success');
}
